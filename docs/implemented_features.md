...
- **Icon-based objective HUD** (`src/components/ObjectivePanel.jsx`): three equal panels — objective, moves, score — each a large value over a small caption, replacing text pills that read `Score: 1,240 / 3,000` and `Jelly: 38`. Those had to be parsed as sentences to answer the only question a player has mid-move: how much is left. Both objective types now report the *remaining* amount counting down to zero, so the number means the same thing on every level; the panel turns green at zero. The jelly icon is drawn with the same `rgba(56,189,248,…)` as `.jelly-overlay` on the board so the HUD and the tile are visibly the same thing. All three panels are `aria-hidden`: the existing `role="status"` live region already reports these values, and duplicating them made every board update announce twice. The value font size is derived from `--board-size` rather than the viewport, so a five-digit score cannot overflow its panel on a narrow phone.
