...
const VOICE_FOR_BANNER = {
  [BANNER.sweet]: 'sweet',
  [BANNER.tasty]: 'tasty',
  [BANNER.delicious]: 'delicious',
  [BANNER.divine]: 'divine',
};
...
describe('getAnnouncement', () => {
  ...
  it('escalates with plain cascade depth', () => {
    expect(getAnnouncement({ cascadeCount: 2 }).voiceKey).toBe('tasty');
    expect(getAnnouncement({ cascadeCount: 3 }).voiceKey).toBe('delicious');
    expect(getAnnouncement({ cascadeCount: 4 }).voiceKey).toBe('divine');
    expect(getAnnouncement({ cascadeCount: 9 }).voiceKey).toBe('divine');
  });

  it('treats a one-special swap as tasty and a two-special combo as the big line', () => {
    expect(getAnnouncement({ specialCount: 1 }).voiceKey).toBe('tasty');
    expect(getAnnouncement({ specialCount: 2 }).voiceKey).toBe('divine');
  });
  ...
  it('lets a special-candy swap outrank cascade depth', () => {
    ...
    expect(getAnnouncement({ specialCount: 1, cascadeCount: 5 }).voiceKey).toBe('tasty');
  });
  ...
  it('is safe to call with no arguments', () => {
    expect(getAnnouncement()).toEqual({ banner: BANNER.sweet, voiceKey: 'sweet' });
  });
});
